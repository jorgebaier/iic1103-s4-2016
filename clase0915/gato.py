...
## retorna la ficha del ganador del tablero, de haber un ganador, o 0 de no haber ganador
    ...
    for i in [0,1,2]:  #range(3)
        if tablero[i][0]==tablero[i][1]==tablero[i][2] and tablero[i][2]!=0:
            return tablero[i][0]

    for i in [0,1,2]:  #range(3)
        if tablero[0][i]==tablero[1][i]==tablero[2][i] and tablero[2][i]!=0:
            return tablero[0][i]

    if tablero[0][0] == tablero[1][1] == tablero[2][2] and tablero[1][1]!=0:
            return tablero[1][1]

    if tablero[0][2] == tablero[1][1] == tablero[2][0] and tablero[1][1]!=0:
            return tablero[1][1]
...
def tablero2str(tablero):
    tab_str = ''
    tab_str = tab_str + "|".join([ficha2str(x) for x in tablero[0]]) + '\n'
    tab_str = tab_str + "-"*5 + '\n'
    tab_str = tab_str + "|".join([ficha2str(x) for x in tablero[1]]) + '\n'
    tab_str = tab_str + "-"*5 + '\n'
    tab_str = tab_str + "|".join([ficha2str(x) for x in tablero[2]]) + '\n'
    return tab_str
